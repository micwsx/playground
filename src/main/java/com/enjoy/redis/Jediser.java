...
public class Jediser {
	...
	public Jediser() {
		if (pool == null) {
			...
			pool = new JedisPool(config, this.ip, this.port, timeout, this.auth);
		}
	}
	...
	public Object evalsha(String script, List<String> keys, List<String> args){
		...
		try {
			jedis = pool.getResource();
//			res = jedis.evalsha(jedis.scriptLoad(script), Arrays.asList("localhost"), Arrays.asList("10", "2"));
			res = jedis.evalsha(jedis.scriptLoad(script), keys,args);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (jedis!=null) {
...
//			res = jedis.eval(script, keys, args);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (jedis!=null) {
